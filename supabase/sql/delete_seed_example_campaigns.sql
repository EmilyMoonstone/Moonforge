-- ============================================================
-- 004_delete_seed_example_campaigns.sql — Remove example data
...
groups_to_delete as (
  ...
  left join campaigns_to_delete ctd on ctd.id = pc.campaign_id
  where ctd.id is not null
     or (
       g.dungeon_master_user_id = (select user_id from seed_user)
       and g.name in ('Astral Voyagers', 'Ember Cell', 'Hollow Scouts')
     )
),
